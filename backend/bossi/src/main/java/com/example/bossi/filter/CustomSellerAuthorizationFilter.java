...
import com.example.bossi.config.auth.CustomSellerDetails;
...
@Slf4j
public class CustomSellerAuthorizationFilter extends OncePerRequestFilter { // 요청 당 한번만 실행되록 보장되는 필터
    // 권한 요청을 하고 있을때 필터 발생

    private final SellerRepository sellerRepository;

    private final JwtSellerTokenService jwtService;

    private GrantedAuthoritiesMapper authoritiesMapper = new NullAuthoritiesMapper();

    public CustomSellerAuthorizationFilter(JwtSellerTokenService jwtService, SellerRepository sellerRepository) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        // 패턴이 제대로 안됨.
        if(request.getServletPath().equals("/api/v1/seller/login") ) {
            log.info("nonono!");
            ...
            if(authorizationHeader != null && authorizationHeader.startsWith(JwtProperties.TOKEN_PREFIX)){
                try {
                    ...
                    JwtSellerTokenService jwtTokenService = new JwtSellerTokenService(sellerRepository);
                    ...
                    Seller memberEntity = sellerRepository.findSellerByEmail(email);
                    CustomSellerDetails principalDetails = new CustomSellerDetails(memberEntity);
